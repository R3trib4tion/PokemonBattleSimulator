...
class Battle:
    ...
    def fight(self):
          ...
          # Prints out all the information about the pokemon that are battling
          print("Pokemon Name: {}" .format(self.p1.name))
          time.sleep(0.6)
          print("Attack Stats: {}" .format(self.p1.attack))
          time.sleep(0.6)
          print("Defense Stats: {}\n" .format(self.p1.defense))
          time.sleep(0.6)

          print(" VS \n")

          print("Pokemon Name: {}".format(self.p2.name))
          time.sleep(0.6)
          print("Attack Stats: {}" .format(self.p2.attack))
          time.sleep(0.6)
          print("Defense Stats: {}\n". format(self.p2.defense))
          time.sleep(0.6)
          print("FIGHT\n")
          print("---------------------------------------------------------------------------------------\n")
          ...
          p1_attack2_3 = 0.01*(p1_attack)

          p2_attack2_1 = 0.045*(p2_attack)
          p2_attack2_2 = 0.02*(p2_attack)
          p2_attack2_3 = 0.01*(p2_attack)


          # If the pokemon speed is faster than the opponent
          if self.p1.speed > self.p2.speed:
              print("Since {} is faster than {}, {} attacks first".format(self.p1.name, self.p2.name, self.p1.name))
              ...
              print("Name of pokemon attacking: ", self.p1.name)
              print("Name of attack: ", P1_attackName)
              print("attack type: ", P1_moveType)
              if (P1_moveType in P2_weaknesseslist) and (P1_moveSpecial == False):
                  p1_attack3 = 2*p1_attack
                  print("If true, its a special attack: ", P1_moveSpecial)
                  print("attack damage ", (p1_attack3))

                  if ((p1_attack3) - p2_def) > 0:
                    print("Real Attack damage = ", (p1_attack2_1))
                    self.p2.reductionHealth(p1_attack2_1)
                  elif ((p1_attack3) - p2_def) == 0:
                    print("Real attack damage = ", (p1_attack2_2))
                    self.p2.reductionHealth(p1_attack2_2)
                  elif ((p1_attack3) - p2_def) < 0:
                    print("Real Attack damage = ", (p1_attack2_3))
              ...
              print("{} and his health {}" .format(self.p2.name, self.p2.health))
              self.p1.exp((mean([self.p2.attack, self.p1.defense])))
              print("{} and his exp gained {} \n" .format(self.p1.name, self.p1.expe))
          ...
          while (self.p1.health > 0) and (self.p2.health > 0):
              ...
              print("Name of pokemon attacking: ", self.p2.name)
              print("Name of attack: ", P2_attackName)
              print("attack type: ", P2_moveType)
              if (P2_moveType in P1_weaknesseslist) and (P2_moveSpecial == False):
                  P2_attack3 = 2*p2_attack
                  print("If true, its a special attack: ", P2_moveSpecial)
                  print("attack damage: ", (P2_attack3))

                  # Player 2 moves #####
                  if ((P2_attack3) - p1_def) > 0:
                    print("Real Attack damage = ", (p2_attack2_1))
                    self.p1.reductionHealth(p2_attack2_1)
                  elif ((P2_attack3) - p1_def) == 0:
                    print("Real attack damage = ", (p2_attack2_2))
                    self.p1.reductionHealth(p2_attack2_2)
                  elif ((P2_attack3) - p1_def) < 0:
                    print("Real Attack damage = ", (p2_attack2_3))
              ...
              print("{} and his health {}\n" .format(self.p1.name, self.p1.health))
              ...
              print("{} and his health {} \n" .format(self.p2.name, self.p2.health))
              ...
              print("The experiences of both pokemons:")
              print("{} and his exp {}" .format(self.p1.name, self.p1.expe))
              print("{} and his exp {}\n" .format(self.p2.name, self.p2.expe))
...
while t < rounds:
    ...
    R2 = Pokemon("Rayquaza", 105, 180, 180, 100, 115, 0, 2, 0, "DRAGON", {"Twister":["DRAGON", True], "Scary Face":["NORMAL", False], "Ancient Power":["ROCK", True], "Crunch":["DARK", False], "Air Slash":["DARK", True]}, ["FIRE", "WATER", "ELECTRIC", "GRASS", "FIGHTING", "BUG"], ["ICE","DRAGON", "GHOST", "FAIRY"])
    ...
    while i > 1:
        ...
        if (A.health > 0) and (B.health <= 0):
            ...
            A.lvl()
            print("'{}' faints" .format(B.name))
            print("Winner is '{}' with {} health" .format(A.name, A.health))
            print("{} gains {} experience and at {} health" .format(A.name, A.expe, A.health))
            print("{} is level {}\n".format(A.name, A.lvl_tracker))
            print("---------------------------------------------------------------------------------------------------------\n")
            ...
            B.lvl()
            print("'{}' faints" .format(A.name))
            print("Winner is '{}' with {} health" .format(B.name, B.health))
            print("{} gains {} experience and at {} health" .format(B.name, B.expe, B.health))
            print("{} is level {}\n".format(B.name, B.lvl_tracker))
            print("---------------------------------------------------------------------------------------------------------\n")
            lst_Pokemons2.remove(A)

        elif (B.health <= 0) and (A.health <= 0):
            time.sleep(0.7)
            print("both '{}' and '{}' faints" .format(A.name, B.name))
            print("---------------------------------------------------------------------------------------------------------\n")
            lst_Pokemons2.remove(A)
            lst_Pokemons2.remove(B)
            k -= 1
